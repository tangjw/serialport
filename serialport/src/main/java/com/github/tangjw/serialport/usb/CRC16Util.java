...
public final class CRC16Util {
    ...
    //滚轴为CRC-16/CCITT  该方法返回为10进制，需转为16进制
    public static String getCRC16Ccitt(byte[] pSrcData) {
        int crc_reg = 0x0000;
        for (byte pSrcDatum : pSrcData) {
            crc_reg = CRC16_ccitt_table[(crc_reg ^ pSrcDatum) & 0xFF] ^ (crc_reg >> 8);
        }
        return Integer.toHexString(crc_reg);
    }


    //升降机和继电器为CRC-16/MODBUS
    public static String getCRC16Modbus(byte[] bytes) {
        ...
        //得出的数据需要进行翻转处理
//        Timber.e("CRC " + (Arrays.toString(buffer.array())));
//        Timber.e("CRC % 256 = " + (CRC % 256));
//        Timber.e("CRC / 256 = " + (CRC / 256));

        String hexString = Integer.toHexString(CRC % 256);
        String hexString1 = Integer.toHexString(CRC / 256);
        String crcHex = hexString + hexString1;
        return crcHex;
    }

    public static void insertCRC16Modbus(byte[] bytes) {
        int CRC = 0x0000ffff;
        int POLYNOMIAL = 0x0000a001;
        int i, j;
        ...
        bytes[bytes.length - 2] = (byte) (CRC % 256);
        bytes[bytes.length - 1] = (byte) (CRC / 256);
    }
}